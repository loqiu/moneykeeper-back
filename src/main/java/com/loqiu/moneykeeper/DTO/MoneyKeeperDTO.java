...
public class MoneyKeeperDTO {
    ...
    @Schema(description = "分类名称", example = "餐饮", hidden = true)
    @ExcelProperty(value = "分类名称", index = 0)
    @ColumnWidth(15)
    private String categoryName;

    @Schema(description = "类型", example = "支出", hidden = true)
    @ExcelProperty(value = "类型", index = 1)
    @ColumnWidth(10)
    private String type;

    @Schema(description = "金额", example = "99.99", hidden = true)
    @ExcelProperty(value = "金额", index = 2)
    @ColumnWidth(15)
    private BigDecimal amount;

    @Schema(description = "交易日期", example = "2024-01-01", hidden = true)
    @ExcelProperty(value = "交易日期", index = 3)
    @DateTimeFormat("yyyy-MM-dd")
    @ColumnWidth(15)
    private LocalDate transactionDate;
    
    @Schema(description = "更新时间", example = "2024-01-01 12:00:00", hidden = true)
    @ExcelProperty(value = "更新时间", index = 4)
    @DateTimeFormat("yyyy-MM-dd HH:mm:ss")
    @ColumnWidth(20)
    private LocalDateTime updatedAt;

    @Schema(description = "备注", example = "午餐费用", hidden = true)
